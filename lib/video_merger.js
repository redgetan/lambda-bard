...
VideoMerger.prototype.concatSegments = function() {
  ...
  return this.segmentUrlsFromCharacterScene(this.text, this.character_token, this.scene_token
    ...
    outputFileAbsolutePath = this.generateOutputFileAbsolutePath(segment_urls);

    return self.fetchSegments(segment_urls);
...
VideoMerger.prototype.fetchCommand = function(segment_urls) {
    return "mkdir -p " + this.DOWNLOADED_SEGMENTS_DIR + 
           " && cd " + this.DOWNLOADED_SEGMENTS_DIR + 
           " && printf '" + segment_urls.join("\n") + 
...
VideoMerger.prototype.mergeCommand = function(segmentPaths, outputFileAbsolutePath) {
  ...
  var concatFilterMid  = localSegmentPaths.map(function(segment_url, index){ return " [" + index + ":v] " + "[" + index + ":a] "; }).join("");
...
VideoMerger.prototype.getLocalSegmentPaths = function(segment_urls) {
  return segment_urls.map(function(url){ 
    return this.DOWNLOADED_SEGMENTS_DIR + "/" + url.split("/").slice(-1)[0]; 
  }.bind(this));
};

VideoMerger.prototype.generateOutputFileAbsolutePath = function(segment_urls) {
  var outputFile = uuid.v4() + ".mp4";
  return "/tmp/" + this.NAMESPACE + "/" + outputFile;
};

VideoMerger.prototype.fetchSegments = function(segment_urls, cb) {
  var fetchCmd = this.fetchCommand(segment_urls);
